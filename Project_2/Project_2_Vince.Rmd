...
```{r}
# remove Member ID
df <- df.members_Y1 %>% select(-MemberID)
...
# cluster
km.out <- kmeans(df,5,nstart=20)


def.par <- par(no.readonly = TRUE, mar = c(8.1, 4.1, 4.1, 2.1)) # save default, for resetting...
layout(t(1:5)) # 4 plots in one
for(i in 1:5) barplot(km.out$centers[i,], main=paste("Cluster", i), las = 2)
```

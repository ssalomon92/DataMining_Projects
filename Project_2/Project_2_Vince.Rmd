...
df.members_Y1_h <- df.members_Y1_scaled %>%
  ...
  filter(cluster==3) %>%
  select(age,male,female,Max_Charlson,Min_Charlson,char_diff)

hc.complete <- hclust(dist(df.members_Y1_h),method='complete')

hc.average <- hclust(dist(df.members_Y1_h),method='average')

hc.single <- hclust(dist(df.members_Y1_h),method='single')
```

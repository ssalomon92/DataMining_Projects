...
```{r}
ggplot(data=df, aes(x=reorder(df$placesvc,df$placesvc,
                     function(x)-length(x)))) +
  geom_bar() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
...
#### Discussion
This breakdown appears to hav good data quality, but I am curious way there is not a category for days since first claim of >12 months. Does a claim guarentee you will have another claim wihtin a year? The breakdown shows that after the first claim you have a higher chance to return. 
```{r}
...
```{r}
ggplot(data=df, aes(x=AgeAtFirstClaim, y=CharlsonIndex)) +
  geom_count() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(y='Age at First Claim', x='Charlson Index' , title='Charlson Index across Age at First Claim')
```

#### Discussion
The plot above shows the correlation between age at first claim and charlson index. It appears that they are positiviley correlated. This is possible to determine from the ordinal nature of the categorical variables. The positive correlation is as expected considering the description of charlson index in the data dictionary. While the variables are correlated the correlation does not appear extreme. The age groups below 40 seem more similar to one another when compared to groups greater than 40. I do not think the correlation is reason for concern or a good enough reason to throw out either of the variables.
...
```{r}
sum(sort(unique(df.Members_Y1$MemberID)) == sort(unique(df.Claims_Y1$MemberID)) &  sort(unique(df.Claims_Y1$MemberID)) == sort(unique(df.DayInHospital_Y2$MemberID)))
```
## 3. Data Preperation

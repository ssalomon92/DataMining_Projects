...
#### Discussion
The barplot above shows the breakdown for place of service. Remember this variable was dominated by Office. This variable does not appear to have the as much as a possibility for overlap as specialty. This variable has potential for being a good predictor.
...
#### Discussion
The breakdown for length of shows a clear data quality issue. This is consistent with our breakdwon of the Claims_Y1 dataset showing ~95% of the length of stay variable is empty string. While even though alot of these values are suppressed converted the days to numerical, and aggregating by member Id might show improvment. If this is not successful other options will need to be considered.
...
#### Discussion
The primary condition group does not appear to show anything extremely interesting other than some groups are more popular than others. A potential variable to create is a indicator if a member id has used multiple primary condition groups.
...
#### Discussion
The charts above show interactions between specialty, days since first service, and age at first claim. Age at first claim was merged to the Claims_Y1 dataset. Age at first claim is repeadeted for the specific memberid that it corresponds with within the Claims_Y1 dataset. 

This chart shows the concentraion of the claims as it relates to these categorical variable e.g. the observations that have higher age at first claim concentrate in generale practice, internal, and laboratory for the specialty variable. This is concentraion for the specialty variable also occurs for lo days since first claim greater than 1 month. The 0-1 months category for days since first claim is the category that seems least effected when varying specialty. The 0-1 mont category is also least effected when comparing across age at first claim i.e. if the days since first claim is high more than likely the age when the first claim was made is also high.

The first of two main interpretations of this plot is that if the days since first claim is greater than 1 month or the age at first claim is high the specialty is most likely general practice, internal, or laboratory. A fair assumption is that higher age at first claim corresponds to older individuals when compared to the younger age at first claim groups. That makes the first interpreation logical. Older individuals with longer periods between claims more than likely have regularly scheduled appointments that would take place at these locations. The second interpretation is that lower age at first claim have lower days since first claim and varying specialty. This makes sense that younger individuals have more variable claims related to specialty, and it appears that there next claim occurs faster. This makes sense that someone who is willing to make a claim earlier on in life will have a shorter period before their next claim.
...
ggplot(data=df.only_f, aes(x=Var1,y=Freq,fill=CharlsonIndex)) +
  ...
  labs(y='Count', x='Age' , caption='Figure 14: Bar graph with charlson index distribution for breakdown  of age when first claim variable')
...
ggplot(df.Members_Y1, aes(Claims_During_Y1)) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1), plot.caption=element_text(hjust =-0.1,size=10)) +
  labs(y='Count', x='Claims', caption='Figure 16: Distribution of claims per member ID for year 1')
```
</center>
...
Figure 17: Distribution of mean months since first claim per member ID viewed, and average mean months viewed across age at first claim for year 1

#### Discussion
The mean months since first service variable is zero inflated. This is in part because first claims are coded the same as being within 1 month of the first claim. As mentioned before we may need to try to fix this. Viewing the boxplot on the right we can see that on average those whose age at first claim is higher usually results in larger time since first claims. This is similar to our thoughts above that those with age at at first claim are likely to have less claims, and now we can see they have on average longer time between claims. Since this variable is since the first claim this could be due to the fact they have more claims, and claims take time.
...
Figure 18: Correlation between the days in hospital during year 2 and counts of vendor (left) & provider (right)

#### Discussion
The count of vendor and the count of provider per member id varibles were created. The variables are very similar to one another, and there relationship to days in hospital during year 2 is also similar. This could be due to the fact that switchin providers mostly results in changing vendors. This chart also shows how the capping of the days in hospital during year 2 can cause for un natural correlation with variables.
